...
- The pool bed is read through the water, sampled from the ground map and falling away with depth. Without it the body was a smooth tone ramp, which left the pool looking like dusty glass whenever nothing lit it from nearby; a shallow pool is mostly read by the litter visible through it. A fine chop band on unrelated, non-axis-aligned frequencies breaks moonlight into a sheen rather than one even sheet.
- Ripples are displacement of the water itself, with no drawn geometry over it. Wavefronts are warped off true by a smooth function of direction, since a front measured as plain radius is a perfect circle and real water never makes one; the warp is built from the offset's own unit-vector components, whose sweep supplies the harmonics, so no angle needs computing.
...
- A passing wave bends the bed seen through the surface, breaks up the reflection, and shades the surface itself, brightening along one face and darkening along the other. That last channel does most of the work over the middle of the pool, where the bed is too deep to read and the reflection is weak at that angle.
...
- The snail is wet like everything else around it: its shell and skin carry a water film in the clearcoat, broken up by their own roughness maps, where previously they were the only fully matte surfaces left in the scene.
